...
To establish the fundamental stability of the actuators, tuning began with isolated step responses applied exclusively to the inner velocity loops. A step input represents an instantaneous acceleration demand—the harshest possible condition for the stepper motors and the 3D-printed PLA mechanical linkages.

The objective during this phase was not smooth movement, but raw responsiveness and stability. The Proportional (P) and Integral (I) gains of the velocity loop were adjusted to achieve a critically damped response with a rapid rise time, ensuring the motor could reach the commanded velocity without ringing, oscillating, or stalling against its physical 0.28 Nm limit. By securing a highly responsive inner loop, the system adheres to the bandwidth separation principle (often referred to as the "Rule of 10"), guaranteeing that the inner velocity dynamics settle significantly faster than the outer position loop updates.

## Routine based Tuning
With the inner velocity loop stabilized, the outer position loop was tuned using the generated Minimum Jerk (Quintic Polynomial) S-Curve trajectory rather than a raw step input.

Attempting to tune a robotic position loop with a step response typically forces the use of artificially low P-gains to prevent violent overshoot. However, the S-Curve trajectory cleanly bypasses this limitation by smoothly updating the target position every fundamental time step. This keeps the instantaneous error incredibly small throughout the entire motion profile:
e(t)=Target(t)−Actual(t)

Because e(t) remains bounded and small, the positional proportional gains could be pushed significantly higher. This creates a mechanically "stiff" system capable of immediate corrections without the risk of overshoot. This high-gain configuration effectively eliminates phase lag (tracking error), ensuring the physical manipulator tightly hugs the mathematical path generated by the trajectory planner, fully exploiting the geared mechanical advantages of the shoulder and elbow joints.

**Tuning tests, base,shoulder and elbow respectively with their torques**
![tuning_results](/pics/tuning_results.png)
